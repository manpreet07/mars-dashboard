...
const getRoverManifestDataAndPhotos = (async (url) => {
	try {
...
const roverData = (async () => {
	...
	const roverManifest = await getRoverManifestDataAndPhotos(`${baseUrl}/rovers/${name.toLowerCase()}`);
	const roverPhotos = await getRoverManifestDataAndPhotos(`${baseUrl}/rovers/${name.toLowerCase()}/photos`);
	...
	const html = `<h3>Rover: ${roverName}</h3>
				  <p>Launch Date:<b>${rover.launch_date}</b> Landing Date:<b>${rover.landing_date}</b> Status:<b>${rover.status}</b> Most recent photos taken on:<b>${rover.max_date}</b>Total photos taken:<b>${rover.total_photos}</b></p>`
	let innerHTML = '';
